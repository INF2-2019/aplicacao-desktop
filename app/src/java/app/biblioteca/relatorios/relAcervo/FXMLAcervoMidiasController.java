...
public class FXMLAcervoMidiasController implements Initializable {
    ...
    @FXML
    private TableColumn<ModelTable, String> col_tempo;
    ...
    static public void consultarBD() {
        try {
            ...
            ResultSet rsM = con.createStatement().executeQuery("select * from midias");
            oblist = FXCollections.observableArrayList();

            while (rs.next()) {
                    if (rs.getString("tipo").equals("MIDIAS") && rsM.next()) {
                        oblist.add(new ModelTable(rs.getInt("id"), rs.getInt("id-campi"), rs.getInt("ano"), rs.getInt("paginas"), rs.getString("nome"), rs.getString("local"), rs.getString("editora"), rsM.getString("subtipo"), rsM.getString("tempo")));
                    }
            }
	...
	if (file != null) {
	    ...
	    PdfWriter.getInstance(my_pdf_report, new FileOutputStream(file.getAbsolutePath() + "\\relatorioAcervoMidias.pdf"));
	    my_pdf_report.open();
	    ...
	    String head[] = {"ID", "CAMPI", "NOME", "LOCAL", "ANO", "EDITORA", "PÁGINAS", "TEMPO", "SUBTIPO"};
	    Font bold = new Font(Font.FontFamily.UNDEFINED, 12, Font.BOLD);

	    Paragraph title = new Paragraph("RELATÓRIO ACERVO MÍDIAS");
		...
		if (rs.getString("tipo").equals("MIDIAS") && rsM.next()) {
		    ...
		    my_report_table.addCell(table_cell);
		    String tempo = rsM.getString("tempo");
		    table_cell = new PdfPCell(new Phrase(String.valueOf(tempo)));
		    my_report_table.addCell(table_cell);
		    String subtipo = rsM.getString("subtipo");
