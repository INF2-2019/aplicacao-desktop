 ...
 * @author Marcelo
 */
public class FXMLAcervoAcademicosController implements Initializable {
    ...
    @FXML
    private TableColumn<ModelTable, String> col_programa;
    ...
    static public void consultarBD(){
        try {
            ...
            ResultSet rsA = con.createStatement().executeQuery("select * from academicos");
            oblist = FXCollections.observableArrayList();
            
                while(rs.next() ){
                    if(rs.getString("tipo").equals("ACADEMICOS") && rsA.next()){
                    oblist.add(new ModelTable(rs.getInt("id"),rs.getInt("id-campi"),rs.getInt("ano"),rs.getInt("paginas"),rs.getString("nome"),rs.getString("local"),rs.getString("editora"), rsA.getString("programa")));
                }}
	...
	col_editora.setCellValueFactory(new PropertyValueFactory<>("editora"));
        ...
        col_programa.setCellValueFactory(new PropertyValueFactory<>("programa"));
	...
	if (file != null) {
	    ...
	    PdfWriter.getInstance(my_pdf_report, new FileOutputStream(file.getAbsolutePath() + "\\relatorioAcervoAcademicos.pdf"));
	    my_pdf_report.open();
	    ...
	    String head[] = {"ID", "CAMPI", "NOME", "LOCAL", "ANO", "EDITORA", "PÁGINAS", "PROGRAMA"};
	    Font bold = new Font(Font.FontFamily.UNDEFINED, 12, Font.BOLD);

	    Paragraph title = new Paragraph("RELATÓRIO ACERVO ACADEMICOS");
		...
		if (rs.getString("tipo").equals("ACADEMICOS") && rsA.next()) {
		    ...
		    my_report_table.addCell(table_cell);
		    String programa = rsA.getString("programa");
		    table_cell = new PdfPCell(new Phrase(String.valueOf(programa)));
		    my_report_table.addCell(table_cell);
		}
	    }
